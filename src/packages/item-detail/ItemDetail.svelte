<script>
  ...
  function oEmded_image(item) {
    let youtubeformat = item.links
      .split(';')
      .find((s) => s.startsWith('video|') && (s.includes('youtube.com') || s.includes('youtu.be')));
  ...
  function wikiUrlForEmbed(item) {
    var wikiurl = item.links
      ...
      .split('|')[1];
    return wikiurl
      .replace('simple.wikipedia.org/', 'simple.m.wikipedia.org/')
  ...
  >
      ...
      {#each item.topics.split(';') as topicname}
        <div class="group inline-flex">
          <a href={'#/topic/' + topicname} class="mr-2 font-bold"
            ...
            <div
              class="w-5 mt-0.25 h-0.5 ml-1 bg-primary group-hover:w-full ease-in-out duration-300"
            />
          </a>
        </div>
      {/each}
    ...
    <div class="mt-10">
      <div class="mb-10 flex flex-wrap  justify-start">
        ...
        >
          ...
          {:else if item.image}
            <div class="">
              <img
                class="mr-5 mb-6 sm:w-44 sm:h-64 transform rounded-md shadow-lg transition duration-300 ease-out hover:scale-105 md:shadow-xl "
                src={item.image}
                alt={item.name}
              />
            </div>
          {:else if item.links.includes('video')}
            <div class="relative mr-5 rounded-lg overflow-hidden shadow-lg">
              <div class="w-80 h-60">
                <img
                  class="h-auto w-80 flex justify-center items-center border-r border-gray-500 relative"
                  src={oEmded_image_ytb_url}
                  alt={item.name}
                />
              </div>
              <div
                ...
                aria-hidden="true"
              >
                <svg class="h-12 w-12 text-indigo-500" fill="currentColor" viewBox="0 0 84 84"
                  ><circle opacity="0.9" cx="42" cy="42" r="42" fill="white" /><path
        ...
        <!-- item details  -->
        <div
          class={(item.image ? 'lg:mt-0' : '') +
            ' mt-10 w-full flex flex-col justify-between flex-1'}
        >
          ...
          <section>
            ...
            <div class="mt-5">
              <sl-rating readonly precision="0.1" value={item.rating} />
            </div>
          </section>
          ...
          <div class="mt-2 mb-6 flex flex-col justify-between">
              ...
              {#each item.links.split(';') as type}
                <sl-button-group>
                  <sl-button
                    ...
                    <sl-icon name="link-45deg" /></sl-button
                  >
                  {#if type.split('|')[2] || type.split('|')[0] === 'book'}
                    <sl-dropdown
                      placement="bottom-end"
                      on:sl-select={(e) => window.open(e.detail.item.value, '_blank')}
                    >
                      <sl-button slot="trigger" size="small" caret>
                        <sl-icon name="cloud-download" />
                      </sl-button>
                      <sl-menu style="width: 200px;">
                        {#if type.split('|')[2] && type.split('|')[2].startsWith('ipfs:')}
                          ...
                          <sl-menu-item
                            value={'https://cloudflare-ipfs.com/ipfs/' +
                              type.split('|')[2].replace('ipfs:', '')}>Cloudflare</sl-menu-item
                          >
                          <sl-menu-item
                            value={'https://ipfs.io/ipfs/' +
                              type.split('|')[2].replace('ipfs:', '')}>IPFS.io</sl-menu-item
                          >
                          <sl-menu-item
                            value={'https://ipfs.infura.io/ipfs/' +
                              type.split('|')[2].replace('ipfs:', '')}>Infura</sl-menu-item
                          >
                          <sl-menu-item
                            value={'https://gateway.pinata.cloud/ipfs/' +
                              type.split('|')[2].replace('ipfs:', '')}>Pinata</sl-menu-item
                          >
                        {/if}

                        {#if type.split('|')[2] && type.split('|')[2].startsWith('doi:')}
                          <sl-menu-item
                            value={'https://sci-hub.se/' + type.split('|')[2].replace('doi:', '')}
                            >On SciHub</sl-menu-item
                          >
                        {/if}
                        ...
                        {#if type.split('|')[0] === 'book'}
                          ...
                          <sl-menu-item
                            value={'http://libgen.rs/search.php?req=' +
                              encodeURIComponent(item.name)}>LibGen</sl-menu-item
                          >
                          <sl-menu-item
                            value={'https://openlibrary.org/search?q=' +
                              encodeURIComponent(item.name)}>OpenLibrary</sl-menu-item
                          >
                          <sl-menu-item
                            value={'https://www.goodreads.com/search?q=' +
                              encodeURIComponent(item.name)}>GoodReads</sl-menu-item
                          >
                        {/if}
                      </sl-menu>
                    </sl-dropdown>
            ...
            <ButtonGroup
              tabs={['Want to learn', 'Finished']}
              currentlySelected={$bookmarks[itemid]}
              on:change={saveStatusToLocalStorage}
            />
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
        ...
        {#if item.creators}
          <div
            class="flex flex-col justify-between items-center gap-1 border border-neutral_dark py-5 px-2"
          >
            <div class="flex flex-col items-center">
