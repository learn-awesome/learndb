  ...
  <div class="relative pt-1">
    {#if showForm == false}
      <div class="bg-primary rounded absolute top-0 right-0">
        <button
          on:click={(e) => (showForm = true)}
          type="button"
          class=" p-2 text-primary_light focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary lg:hidden"
        >
    ...
    {#if showForm}
      <form
        class="w-full p-2 gap-3 mt-10 lg:mt-0 flex flex-col xl:flex-row"
      ...
      >
        <sl-input
          ...
          size="medium"
          clearable
          class="w-full flex-1 border-0 p-0 focus:ring-0"
          value={query.text}
          on:sl-input={(e) => (query.text = e.target.value)}
        >
        ...
        <div class="flex flex-col md:flex-row justify-center items-center gap-3 w-full">
          <sl-select
            class="w-full"
            on:sl-change={(e) => (query.tag = e.target.value)}
            value={query.tag}
          >
          ...
          <sl-select
            class="w-full"
            on:sl-change={(e) => (query.level = e.target.value)}
            value={query.level}
          >
        ...
        <sl-select
          ...
          on:sl-change={(e) => (query.sortby = e.target.value)}
          value={query.sortby}
        >
          <sl-icon name="sort-down-alt" slot="prefix" />
