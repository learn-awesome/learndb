  ...
  function hierarchy(topic_array, parent_id) {
    ...
    for (let i = 0; i < topic_array.length; i++) {
      if (parentids.includes(topic_array[i].parent_id)) {
        tempmap
          .get(topic_array.find((t) => t.name == topic_array[i].parent_id))
          .push(topic_array[i]);
      }
    }
    // console.log(tempmap);
...
<div class="my-2 flex flex-row">
  ...
  <a
    ...
    target="_blank"
    class="underline text-primary hover:font-bold px-2">Help us improve this taxonomy</a
  >
</div>
  ...
  {#each [...map.entries()].sort((t1, t2) => (t1[0].sort_index || 100) - (t2[0].sort_index || 100)) as parent}
    {#if parent[0] !== 'Misc'}
      <a href={'#/topic/' + parent[0].name}>
        <div
          class="rounded shadow-md p-4 break-inside-avoid mb-4 border border-secondary bg-primary_light ease-in-out duration-300 hover:rounded-3xl "
        >
          ...
          {:else}
            <div class="group inline-flex">
              <a href={'#/topic/' + parent[0].name}
                ><span class="mt-1 p-1 text-primary font-extrabold text-lg "
                  >{format_topic_name(parent[0])}</span
                >
                <div
          ...
          <div class="mt-2 flex flex-wrap">
            {#each parent[1].sort((t1, t2) => t1.name.localeCompare(t2.name)) as child}
              <a
                ...
                class="text-primary no-underline hover:underline hover:underline-offset-2 px-2 "
                >{format_topic_name(child)}</a
              >
            {/each}
          </div>
        </div>
    ...
    {#if parent[0] == 'Misc'}
      <div
        class="rounded-lg shadow-md p-4 break-inside-avoid mb-4 border border-secondary bg-primary_light  ease-in-out duration-300 hover:rounded-3xl"
      >
        {#if typeof parent[0] == 'string'}
