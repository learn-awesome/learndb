<script>
  ...
  function findYoutubeUrl(links) {
    let youtubeformat = links
      .split(';')
      .find((s) => s.startsWith('video|') && (s.includes('youtube.com') || s.includes('youtu.be')));
    if (youtubeformat) {
      return youtubeformat.split('|')[1];
    }
  }

  $: io_fetchVideoThumbnailUrl(findYoutubeUrl(item.links), (d) => {
    oEmded_image_ytb_url = d;
  });
</script>
    ...
    <div class="h-36 w-full md:w-64 flex justify-center items-center relative ">
      {#if item.image || oEmded_image_ytb_url}
        <div class="rounded-lg">
          <img
            src={item.image || oEmded_image_ytb_url}
      ...
      {:else}
        <div class="w-full flex justify-center items-center rounded-lg">
          <div class="w-48 h-36 bg-primary" />
          <div
            class="absolute inset-0 w-full h-full flex items-center justify-center"
            aria-hidden="true"
          >
