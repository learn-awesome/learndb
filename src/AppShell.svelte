<script>
  ...
  import AdvancedSearch from './packages/search-advanced/AdvancedSearch.svelte';
...
<div>
  <!-- sticky top bar  -->
  <div
    class="sticky top-0 z-50 flex-shrink-0 flex items-center text-neutral-50 bg-blue-900 shadow h-16"
  >
    <div class="flex items-center">
      {#if isNavDrawerOpen == false}
        <button
          on:click={(e) => (isNavDrawerOpen = true)}
          type="button"
          class="px-4 border-r border-primary_light text-primary_light focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary md:hidden"
        >
      ...
      {#if isNavDrawerOpen}
        <button
          on:click={(e) => (isNavDrawerOpen = false)}
          type="button"
          class="px-4 border-r border-primary_light text-primary_light focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary md:hidden"
        >
      ...
      <div
        class=" py-3 flex flex-col items-center flex-shrink-0 px-4 tracking-wider font-bold group"
      >
        <a href="/" class="">LearnAwesome</a>
        <div
          class="w-1/5 mt-0.25 h-0.5 bg-neutral-50 group-hover:w-full ease-in-out duration-300"
        />
      </div>
    </div>
  ...
  {#if isNavDrawerOpen}
    <div class="relative z-40 md:hidden" role="dialog" aria-modal="true">
      <div
        class="fixed inset-0 bg-neutral_light bg-opacity-75 mt-12"
        on:click={(e) => (isNavDrawerOpen = false)}
      />
